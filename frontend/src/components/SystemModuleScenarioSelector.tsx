...
const SystemModuleScenarioSelector: React.FC<SystemModuleScenarioSelectorProps> = ({
  ...
  const [loading, setLoading] = useState<{ systems: boolean; modules: boolean; scenarios: boolean }>({
    systems: false, modules: false, scenarios: false,
  });

  const labelColor = isDark ? '#94a3b8' : undefined;
  ...
  useEffect(() => {
    const loadModules = async () => {
      if (!state.selectedSystem) { setModules([]); setScenarios([]); return; }
      try {
  ...
  useEffect(() => {
    const loadScenarios = async () => {
      if (!state.selectedModule) { setScenarios([]); return; }
      try {
  ...
  const handleSystemChange = (systemId: number | undefined) => {
    ...
    if (onSelectionChange) onSelectionChange(system, undefined, undefined);
  };
  ...
  return (
    <Form layout="vertical">
      <Space direction="horizontal" size="middle" style={{ width: '100%' }} wrap={true}>
        <Form.Item label={<span style={{ color: labelColor }}>系统</span>} required style={{ flex: 1, minWidth: 200, marginBottom: 0 }}>
          <Select
            placeholder="请选择系统"
        ...
        <Form.Item label={<span style={{ color: labelColor }}>功能模块</span>} required={!!state.selectedSystem} style={{ flex: 1, minWidth: 200, marginBottom: 0 }}>
          <Select
        ...
        <Form.Item label={<span style={{ color: labelColor }}>功能场景</span>} style={{ flex: 1, minWidth: 200, marginBottom: 0 }}>
          <Select
            placeholder={
          ...
          >
            {scenarios.length === 0 && !loading.scenarios ? (
              <Option value="empty" disabled style={{ color: isDark ? '#64748b' : '#999' }}>
                该模块暂无功能场景
              </Option>
            ) : (

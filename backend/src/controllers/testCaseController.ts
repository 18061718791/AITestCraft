...
export class TestCaseController {
  ...
  async getTestCases(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const filters: { systemId?: number; moduleId?: number; scenarioId?: number; userId?: number | undefined; isAdmin?: boolean } = {};
   ...
   * 按层级获取测试用例（带分页）
   */
  async getTestCasesByHierarchy(req: AuthRequest, res: Response) {
    try {
      const {
        systemId,
        moduleId,

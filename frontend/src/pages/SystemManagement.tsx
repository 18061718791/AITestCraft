  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      if (data) {
        ...
        let url = '';
        if (type === 'system') {
          url = `/system/systems/${data.id}`;
        } else if (type === 'module') {
          url = `/system/modules/${data.id}`;
        } else {
          url = `/system/scenarios/${data.id}`;
        }
        ...
        if (type === 'system') {
          ...
          url = `/system/systems/${parentId}/modules`;
        } else {
          url = `/system/modules/${parentId}/scenarios`;
        }
...
const SystemManagement: React.FC = () => {
  ...
  const fetchTreeData = async () => {
    setLoading(true);
    try {
      const response = await apiClient.get('/system/systems/tree');
  ...
  const handleDelete = async (node: TreeNode) => {
    try {
      let url = '';
      if (node.type === 'system') {
        url = `/system/systems/${node.id}`;
      } else if (node.type === 'module') {
        url = `/system/modules/${node.id}`;
      } else {
        url = `/system/scenarios/${node.id}`;
      }

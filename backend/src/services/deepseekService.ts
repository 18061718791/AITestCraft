...
export class DeepSeekService {
  ...
  constructor() {
    ...
    this.modelType = config.deepseek.modelType;
  ...
  async generateTestCases(
    ...
    const batchSize = 5; // 每批处理5条测试点
    const allTestCases: TestCase[] = [];
    const totalBatches = Math.ceil(testPoints.length / batchSize);
    
    logger.info('deepseek_service', 'generating_test_cases_batched', {
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      const end = Math.min(start + batchSize, testPoints.length);
      const batchPoints = testPoints.slice(start, end);
      
      logger.info('deepseek_service', 'processing_batch', {
        batchIndex: batchIndex + 1,
        totalBatches,
        batchPointsCount: batchPoints.length
      });

      try {
        const batchCases = await this.generateTestCasesBatch(
          batchPoints, 
          system, 
          module, 
          scenario,
        ...
        const numberedCases = batchCases.map((testCase, index) => ({
          ...testCase,
          number: `Test${String(start + index + 1).padStart(4, '0')}`
        }));
        ...
        // 通知进度
        if (onProgress) {
          onProgress(batchIndex + 1, totalBatches, numberedCases);
        }
        
        logger.info('deepseek_service', 'batch_completed', {
          batchIndex: batchIndex + 1,
          totalBatches,
        ...
        const errorMessage = this.getErrorMessage(error);
        logger.error('deepseek_service', 'batch_processing_failed', error, {
          batchIndex: batchIndex + 1,
          totalBatches
        });
  ...
  private async generateTestCasesBatch(
    testPoints: string[], 
    system?: string, 
    module?: string, 
    scenario?: string,
    ...
    logger.info('deepseek_service', 'generating_batch_test_cases', {
      batchIndex,
      totalBatches,
      testPointsCount: testPoints.length,
      promptLength: prompt.length
  ...
  private async makeAPICall(prompt: string): Promise<string> {
    ...
    for (let attempt = 1; attempt <= config.deepseek.maxRetries; attempt++) {
      try {
        ...
        logger.info('deepseek_service', 'api_response_received', {
          attempt,
    ...
    logger.error('deepseek_service', 'api_call_max_retries_exceeded', {
      maxRetries: config.deepseek.maxRetries,
      lastError: lastError?.message,
      lastErrorStack: lastError?.stack,
      lastErrorCode: (lastError as any)?.code,
      lastErrorResponse: (lastError as any)?.response?.data,
      lastErrorStatus: (lastError as any)?.response?.status
    });
  ...
  // 从配置服务加载配置
  async loadConfigFromService(): Promise<boolean> {
    try {
      // 延迟导入，避免循环依赖
      const { configService } = await import('./configService');
      
      // 获取大模型配置
      const apiKeyConfig = await configService.getConfigByKey('LLM_API_KEY');
      const apiUrlConfig = await configService.getConfigByKey('LLM_BASE_URL');
      const modelTypeConfig = await configService.getConfigByKey('LLM_TYPE');
      const timeoutConfig = await configService.getConfigByKey('LLM_TIMEOUT');
      const maxRetriesConfig = await configService.getConfigByKey('LLM_MAX_RETRIES');
      const retryDelayConfig = await configService.getConfigByKey('LLM_RETRY_DELAY');
      
      // 更新配置
      if (apiKeyConfig) this.apiKey = apiKeyConfig.config_value;
      if (apiUrlConfig) this.apiUrl = apiUrlConfig.config_value;
      if (modelTypeConfig) this.modelType = modelTypeConfig.config_value;
      
      // 更新客户端实例
      this.client = axios.create({
        baseURL: this.apiUrl,
        timeout: timeoutConfig ? parseInt(timeoutConfig.config_value) : config.deepseek.timeout,
        headers: {
          'Authorization': `Bearer ${this.apiKey}`,
          'Content-Type': 'application/json',
        },
      ...
      logger.info('DeepSeek configuration loaded from service');
      return true;
    } catch (error) {
      logger.error('Failed to load DeepSeek configuration from service', error);
      return false;
    }
  }
}

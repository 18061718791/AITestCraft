  ...
  private async parseExcel(file: Express.Multer.File): Promise<TestCaseRow[]> {
    ...
    worksheet.eachRow((row, rowNumber) => {
      ...
      rows.push({
        ...
        priority: this.parsePriority(values[11]?.toString()), // 优先级 (列11)
        status: this.parseStatus(values[10]?.toString()),     // 状态 (列10)
        tags: values[9] ? values[9].toString().split(',').map((tag: string) => tag.trim()) : undefined, // 标签 (列9)

    ...
    return await prisma.$transaction(async (tx: any) => {
      ...
      if (row.moduleName?.trim()) {
        ...
        if (systemId) {
          ...
          if (!module) {
            // 如果没有找到同名模块，且没有系统ID，跳过模块创建
            importLogger.warn('无法创建模块，缺少系统ID', {
      ...
      if (row.scenarioName?.trim()) {
        // 如果提供了模块ID，使用模块ID查找或创建场景
        if (moduleId) {
          ...
          if (!scenario) {
            // 如果没有找到同名场景，且没有模块ID，跳过场景创建
            importLogger.warn('无法创建场景，缺少模块ID', {

...
class NotificationService {
  ...
  notifyCasesProgress(sessionId: string, data: {
    taskId: string;
    progress: number;
    ...
    if (!roomExists || roomSize === 0) {
      ...
      allSockets.forEach((socket) => {
        const socketSessionId = socket.handshake.query['sessionId'];
        if (socketSessionId === sessionId) {
          socket.emit('cases-progress', data);
        }
      });
    } else {
      this.io.to(sessionId).emit('cases-progress', data);
    }
    
    logger.debug('websocket', 'cases_progress_notification_sent', {
      sessionId,
      event: 'cases-progress',
      data: {
        taskId: data.taskId,
  ...
  notifyVisionFallback(sessionId: string, data: { taskId: string }): void {
    if (!this.io) return;
    ...
    if (roomExists && roomSize > 0) {
      this.io.to(sessionId).emit('vision-fallback', data);
    } else {
      const allSockets = this.io.sockets.sockets;
      allSockets.forEach((socket) => {
        const socketSessionId = socket.handshake.query['sessionId'];
        if (socketSessionId === sessionId && socket.connected) {
          socket.emit('vision-fallback', data);
        }
      });
    }

    logger.info('websocket', 'vision_fallback_notification_sent', {
      sessionId,
      taskId: data.taskId,
    });
  }
   ...
   * 用于显示图片解析、文档解析等详细步骤
   */
  notifyProcessingLog(sessionId: string, data: {
    ...
    if (!this.io) {
      logger.error('websocket', 'processing_log_notification_failed', {
        reason: 'service_not_initialized',
        sessionId,
    ...
    // 如果房间存在，发送到房间
    if (roomExists && roomSize > 0) {
      this.io.to(sessionId).emit('processing-log', logData);
      sent = true;
      logger.debug('websocket', 'processing_log_sent_to_room', {
        sessionId,
        roomSize,
      });
    }

    // 同时广播到所有匹配的socket（包括房间外的）
    const allSockets = this.io.sockets.sockets;
    allSockets.forEach((socket) => {
      const socketSessionId = socket.handshake.query?.['sessionId'];
      if (socketSessionId === sessionId && socket.connected) {
        socket.emit('processing-log', logData);
        sent = true;
      }
    });

    if (!sent) {
      logger.warn('websocket', 'processing_log_no_recipient', {
        sessionId,
        roomExists,
        roomSize,
    ...
    logger.debug('websocket', 'processing_log_notification_sent', {
      sessionId,
      level: data.level,
